...
struct WebView: NSViewRepresentable {
    ...
    class Coordinator: NSObject, WKNavigationDelegate, WKScriptMessageHandler {
        ...
        func webView(_ webView: WKWebView, didStartProvisionalNavigation navigation: WKNavigation!) {
            // Clean up the previous page's setInterval before navigating away
            webView.evaluateJavaScript(
                "if(window.__urlAnalysisIntervalId){clearInterval(window.__urlAnalysisIntervalId);window.__urlAnalysisIntervalId=null;}",
        ...
        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
            ...
            Task { @MainActor in
                ...
                // 1.5s is sufficient — the JS performance monitor already fires at 1s and 3s post-load,
                // and the setInterval keeps collecting late resources every 5s.
    ...
    (function() {
        ...
        // Also capture on demand — store interval ID for cleanup
        window.__urlAnalysisIntervalId = setInterval(captureResources, 5000);
    ...
    func makeNSView(context: Context) -> WKWebView {
        ...
        // Enable developer extras only in debug builds to prevent end-user
        // access to the Web Inspector (information disclosure risk).
        #if DEBUG
